...
#@django.views.decorators.cache.cache_page(30)
def qft(request):
    # quick feature test
    if cache.cache.get('time_now_qft') == None:
        print("DEBUG",'读取了缓存')
        time_now_qft=timezone.now()
        cache.cache.set('time_now_qft',time_now_qft)
    else:
        print("DEBUG",'读取了缓存')
        time_now_qft=cache.cache.get('time_now_qft')


    return HttpResponse(time_now_qft)
